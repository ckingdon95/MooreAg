...
write.csv(usg2_population, "../data/FUND params/usg2_population.csv")

usg2_income = USGscen$USG2$ref_gdp_F
write.csv(usg2_income, "../data/FUND params/usg2_income.csv")

# 3. Save DICE temperature and pulsed temperature pathways

dice_temp = USGscen$USG2$climate$temp    # from 2000 to 2300 by tens
write.table(dice_temp, "../data/DICE climate output/dice_temp.csv", sep=",", row.names = FALSE, col.names = FALSE)

dice_temp_pulse = USGscen$USG2$climate$temppulsed
write.table(dice_temp_pulse, "../data/DICE climate output/dice_temp_pulse.csv", sep=",", row.names = FALSE, col.names = FALSE)
